...
trait IntHeap extends Heap {
  ...
  def sort =scala.util.Sorting
  override def insert(x: Int,h: H) : H
  def delMin (inh:H,outh:List[Int]): (H ,List[Int]) =
    if(inh == empty) (empty,outh)
    else if (deleteMin(inh)==empty) (empty,outh:+findMin(inh))
    else {
	    val out = outh:+findMin(inh)
