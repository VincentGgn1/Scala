...
case class Block(b1: Pos, b2: Pos) {
	...
	def neighbors: List[(Block, Move)] = {
			var neightbors : List[(Block, Move)] = List()
					//Filling the list in trigonometrcal sens 
					neightbors= neightbors:+( left, Left)
					neightbors= neightbors:+( down, Down)
					neightbors= neightbors:+( right, Right)
					neightbors= neightbors:+( up, Up)  
					return neightbors
	}
	...
	def legalNeighbors: List[(Block, Move)] = {
			var Legalneightbors : List[(Block, Move)] = List()
					for 
					{
						i <-0 to neighbors.size -1
						if (neighbors(i)._1.isLegal)

					} yield Legalneightbors = Legalneightbors:+neighbors(i)
					return Legalneightbors 
	}
